...
class TscSlHAdminController extends Controller
{
    ...
    public function score(TscSlSession $session): View|RedirectResponse
    {
        ...
        return view('admin.tsc-sl-h.score', compact('session'));
    }
    ...
    public function storeScore(Request $request, TscSlSession $session): RedirectResponse
    {
        ...
        return redirect()
            ...
            ->with('success', 'Calificación TSC-SL Hospitalidad guardada correctamente.');
    }
    ...
    {
        $session->load(['candidate.position', 'm3Evaluator']);
        return view('admin.tsc-sl-h.results', compact('session'));
    }
}
