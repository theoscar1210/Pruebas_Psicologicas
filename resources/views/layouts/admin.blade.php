...
<div class="flex h-screen overflow-hidden" x-data="{ sidebarOpen: false }">
    ...
    <aside
        ...
        <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto scrollbar-none">
            ...
            <a href="{{ route('admin.candidates.index', ['filter' => 'evaluacion']) }}"
               @click="sidebarOpen = false"
               class="sidebar-link {{ (request()->routeIs('admin.candidates.index') && request('filter') === 'evaluacion') || request()->routeIs('admin.assessments.*') ? 'active' : '' }}">
                <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="{{ route('admin.perfiles.index') }}"
               @click="sidebarOpen = false"
               class="sidebar-link {{ request()->routeIs('admin.perfiles.*') || request()->routeIs('admin.profile.*') ? 'active' : '' }}">
                <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div class="px-4 lg:px-6 pt-4 space-y-2 flex-shrink-0">
            ...
            @foreach([
                ['success', session('success'), 'text-emerald-500',
                 'M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z'],
                ['error', session('error'), 'text-red-500',
                 'M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z'],
                ['warning', session('warning'), 'text-amber-500',
                 'M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z'],
                ['info', session('info'), 'text-brand-500',
                 'M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z'],
            ] as [$type, $msg, $iconColor, $iconPath])
            @if($msg)
            <div class="alert-{{ $type }}"
                ...
                <button @click="show = false"
                        class="flex-shrink-0 opacity-50 hover:opacity-100 transition-opacity ml-2">
