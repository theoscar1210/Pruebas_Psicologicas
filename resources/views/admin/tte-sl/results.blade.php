...
@section('title', 'TTE-SL — Resultados')
@section('header', 'TTE-SL · Resultados')
...
$dimMeta = [
    'C1' => ['label'=>'Contribución y Participación Activa',       'color'=>'indigo', 'max'=>39],
    'C2' => ['label'=>'Escucha y Receptividad a Ideas Ajenas',     'color'=>'sky',    'max'=>44],
    'C3' => ['label'=>'Apoyo y Soporte Interpersonal',             'color'=>'teal',   'max'=>39],
    'C4' => ['label'=>'Gestión Constructiva del Conflicto',        'color'=>'rose',   'max'=>49],
    'C5' => ['label'=>'Responsabilidad Compartida',                'color'=>'orange', 'max'=>49],
    'C6' => ['label'=>'Adaptabilidad al Estilo de los Demás',      'color'=>'violet', 'max'=>44],
    'C7' => ['label'=>'Orientación al Objetivo Colectivo',         'color'=>'amber',  'max'=>31],
];
$colorBg = [
    'indigo'=>'bg-indigo-500','sky'=>'bg-sky-500','teal'=>'bg-teal-500',
    'rose'=>'bg-rose-500','orange'=>'bg-orange-500','violet'=>'bg-violet-500','amber'=>'bg-amber-500',
];
$colorText = [
    'indigo'=>'text-indigo-700','sky'=>'text-sky-700','teal'=>'text-teal-700',
    'rose'=>'text-rose-700','orange'=>'text-orange-700','violet'=>'text-violet-700','amber'=>'text-amber-700',
];
@endphp
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    ...
    <div class="lg:col-span-2 space-y-5">
            ...
            <div class="card-body">
                <div class="flex items-start justify-between flex-wrap gap-4 mb-5">
                    <div>
                        ...
                        <div class="flex items-baseline gap-2">
                            ...
                            <span class="text-lg text-slate-400">/ 275</span>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-3 gap-4 text-center pt-4 border-t border-slate-100">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">M2 Actitudes / 200</p>
                    </div>
                    <div>
        ...
        <div class="card">
            <div class="card-body">
                <h2 class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4">Calificación Módulo 3 — Escenarios grupales</h2>
                @php
                $scTitles = [
                    1 => 'Conflicto de responsabilidades en entrega crítica',
                    2 => 'Integración de miembro con estilo diferente',
                    3 => 'Decisión grupal que no comparte',
                ];
                @endphp
                <div class="space-y-5">
                    @foreach([1,2,3] as $n)
    ...
    <div class="space-y-4">
        ...
        @php
        $alerts = [];
        $c4 = $dims['C4'] ?? 0; $c1 = $dims['C1'] ?? 0;
        $c7 = $dims['C7'] ?? 0; $c5 = $dims['C5'] ?? 0;
        $c2 = $dims['C2'] ?? 0; $c6 = $dims['C6'] ?? 0;
        $c3 = $dims['C3'] ?? 0;
        if ($c4 < 20 && $c1 > 25) $alerts[] = 'Alta participación con baja gestión del conflicto — posible perfil dominante.';
        if ($c7 < 12 && $c5 < 20) $alerts[] = 'Baja orientación al colectivo y baja responsabilidad compartida — perfil individualista.';
        if ($c2 < 18 && $c6 < 18) $alerts[] = 'No escucha ni se adapta — dificultad para integrarse en equipos diversos.';
        if ($c3 < 16 && $c7 < 12) $alerts[] = 'Bajo apoyo y baja orientación colectiva — posible competitividad disfuncional.';
        @endphp
        @if(!empty($alerts))
        <div class="card border-red-100 bg-red-50/40">
            <div class="card-body">
                <h3 class="text-xs font-semibold text-red-600 uppercase tracking-wider mb-3">Señales de alerta</h3>
                <ul class="space-y-2">
        ...
        <div class="card border-slate-100">
            <div class="card-body">
                ...
                <div class="space-y-1 text-xs">
                    @foreach([
                        ['Sobresaliente','234–275','text-emerald-700 bg-emerald-50','sobresaliente'],
                        ['Alto','193–233','text-brand-700 bg-brand-50','alto'],
                        ['Adecuado','152–192','text-amber-700 bg-amber-50','adecuado'],
                        ['En desarrollo','110–151','text-orange-700 bg-orange-50','en_desarrollo'],
                        ['Por debajo','0–109','text-red-700 bg-red-50','por_debajo'],
                    ] as [$label,$range,$cls,$code])
                    <div class="flex items-center justify-between gap-2 px-2 py-1 rounded {{ $session->performance_level === $code ? $cls.' font-semibold' : '' }}">
                        <span>{{ $label }}</span>
