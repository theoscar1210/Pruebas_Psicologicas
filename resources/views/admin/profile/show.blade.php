            ...
            <div>
                ...
                <div class="space-y-2.5">
                    ...
                    <div class="flex items-center gap-3 text-xs min-w-0">
                        <span class="text-slate-600 font-medium truncate shrink-0 max-w-[40%]">{{ $starLabels[$key] ?? ucfirst(str_replace('_', ' ', $key)) }}</span>
                        <div class="flex gap-1 flex-1 min-w-0">
                            @for($s = 1; $s <= 5; $s++)
                            <div class="flex-1 h-4 rounded {{ $s <= $val ? 'bg-brand-500' : 'bg-slate-100' }}"></div>
                            @endfor
                        </div>
                        <span class="font-bold text-slate-700 shrink-0">{{ $val }}/5</span>
                    </div>
                    @endforeach
                </div>
            </div>
